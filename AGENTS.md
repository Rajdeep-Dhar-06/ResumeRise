...
| 2026-08-22 | Antigravity | **Completed Phase 1**: Implemented new schemas (final_report.py, requests.py), modernized all ChatPromptTemplate prompts, updated parser and vector store to pure text with gemini-embedding-2, standardized imports. Verified with 9/9 passing tests. | schemas/, prompts/, services/, config.py, tests/ |
| 2026-08-22 | Antigravity | **Completed Phase 2**: Implemented scorer.py (70/30 math with half-up rounding), search.py (Tavily search wrapper), augmentation.py (tech/non-tech questions and dynamic study plan). Verified with 6/6 passing tests. | services/scorer.py, services/search.py, services/augmentation.py, tests/test_step2_services.py |
| 2026-08-22 | Antigravity | **Completed Phase 3**: Implemented async orchestrator.py DAG pipeline, FastAPI main.py entrypoint, and route handlers (api/health.py, api/analyze.py). Verified entire test suite with 19/19 passing tests. | orchestrator.py, main.py, api/, tests/test_step3_orchestrator.py |
| 2026-09-01 | Antigravity | **Production Stabilization & Latency Optimization**: Switched default model to `gemini-3.5-flash-lite` (dropping inference latency from 86s to 21s and expanding free tier quota to 500 RPD / 15 RPM). Added BullMQ worker cold-start retry backoff. Fixed frontend transient "failed to load report" banner by ignoring aborted Axios requests in `useInterview.js`. Verified production deployment working. | ai_service/config.py, ai_service/.env, backend/src/queues/worker.js, frontend/src/hooks/useInterview.js |
